...
async function run() {
  ...
  console.log(`Analyzing connections among nodes: ${nodes.join(', ')}`);
  ...
  // 2. Check name-based matches (using the exact rules inside getFuzzyNameGroups)
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = i + 1; j < data.length; j++) {
      ...
      // Rule 3: Jaccard >= 0.80 (with size check)
  ...
  console.log(`\n--- Path from 558463 to 585738 ---`);
  const path = findPath(558463, 585738, new Set(), []);
  if (path) {
    let curr = 558463;
