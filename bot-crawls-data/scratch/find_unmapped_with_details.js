...
async function run() {
  ...
  console.log('Fetching all asset items for HCMC...');
  ...
  console.log('Searching for same plot pairs in same district/ward that failed matching...');
  let failedCount = 0;
  let printLimit = 20;
  ...
  console.log(`\nTotal same-plot pairs that did not auto-group: ${failedCount}`);
  await closeDB();
}
