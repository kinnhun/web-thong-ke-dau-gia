...
async function fixUrls() {
  ...
  // Fix AuctionNotice
  // Regex tìm các URL sai như /thong-bao-cong-khai/561763.html hoặc thiếu slug
  const brokenAuctions = await AuctionNotice.find({
    $or: [
      { sourceUrl: { $regex: /\/thong-bao-cong-khai\/\d+\.html$/ } },
  ...
  const brokenOrgs = await OrgSelection.find({
    $or: [
      { sourceUrl: { $regex: /\/lua-chon-to-chuc\/\d+\.html$/ } },
